...
export async function addAvailablePlay(req, res) {
    ...
    if (token_object.error === false) {
        ...
        if (codeData && codeData.code) {
            ...
            generatedCode = generatedCode || '1071';
        ...
        if (smsRes) {
            ...
            return res.status(400).send({ message: "Mesaj gönderilirken bir hata oluştu" });
        }
    } else {
...
async function increaseAvailablePlay(identity) {
...
async function getUsersData(users) {
    ...
    if (user1.bot == false) {
        const user1Identity = await identityCollection.findOne({ _id: ObjectId(user1.identity) }).catch(err =>
    ...
    if (user2.bot == false) {
        const user2Identity = await identityCollection.findOne({ _id: ObjectId(user2.identity) }).catch(err =>
