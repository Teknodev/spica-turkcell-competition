...
// export async function usersAvailablePlayCount() {
//     if (!db) {
//         db = await database();
//     }
//     const users_collection = db.collection(`bucket_${USER_BUCKET_ID}`);

//      await users_collection.updateMany({available_play_count: {$gte: 4}}, { $set: { available_play_count: 3 }}).catch(err => console.log(err))
