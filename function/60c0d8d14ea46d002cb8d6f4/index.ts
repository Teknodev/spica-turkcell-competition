...
const QUESTIONS_REPORT_BUCKET_ID = process.env.QUESTIONS_REPORT_BUCKET_ID;
        ...
        <table style="width: 100%; border-collapse: collapse;">
            <tbody>
                <tr>
                 ...
                 <tr>
                    <td style="width: 33.3%; font-weight: bold;">Ücretli</td>
                    <td style="width: 33.3%;">${numberWithDot(winLoseData[0].win_paid)}</td>
                    <td style="width: 33.3%;">${numberWithDot(winLoseData[0].lose_paid)}</td>
                </tr>
                 <tr>
                    <td style="width: 33.3%; font-weight: bold;">Ücretsiz</td>
                    <td style="width: 33.3%;">${numberWithDot(winLoseData[0].win_free)}</td>
                    <td style="width: 33.3%;">${numberWithDot(winLoseData[0].lose_free)}</td>
                </tr>
                 <tr>
...
async function questionsReport(reportType, dateFilter) {
    ...
    let questionsData = await Bucket.data.getAll(QUESTIONS_REPORT_BUCKET_ID).catch(e => {
        console.log("ERROR 39", e);
        return res.status(400).send({ message: e });
    });

    questionsData.map(val => (val["sum"] = val.correct_answer + val.wrong_answer));
    ...
    if (reportType == 1 || reportType == 11 || reportType == 22 || reportType == 2) {
        ...
        answersData.forEach(data => {
            ...
            <tr>
             ...
             <td style="width: 14.2%;">${numberWithDot(data.correct_answer_1)}</td>
             <td style="width: 14.2%;">${numberWithDot(data.correct_answer_2)}</td>
             <td style="width: 14.2%;">${numberWithDot(data.correct_answer_3)}</td>
             <td style="width: 14.2%;">${numberWithDot(data.wrong_answer_1)}</td>
             <td style="width: 14.2%;">${numberWithDot(data.wrong_answer_2)}</td>
             <td style="width: 14.2%;">${numberWithDot(data.wrong_answer_3)}</td>
            </tr>`;
        });
        answersHtml = `
        ...
        <table style="width: 100%; border-collapse: collapse;">
            ...
            <tr>
                ...
                <td style="width: 25%;">${numberWithDot(
            answersData[0].correct_answer_1 + answersData[0].wrong_answer_1
        )}</td>
            ...
            <tr>
                ...
                <td style="width: 25%;">${numberWithDot(
            answersData[0].correct_answer_2 + answersData[0].wrong_answer_2
        )}</td>
            ...
            <tr>
                ...
                <td style="width: 25%;">${numberWithDot(
            answersData[0].correct_answer_3 + answersData[0].wrong_answer_3
        )}</td>
    ...
    questionsData.forEach(question => {
        questionsBody += `<tr>
                    <td style="width: 13.3%;">${question.question}</td>
                    <td style="width: 13.3%;">${question.level}</td>
                    <td style="width: 13.3%;">${numberWithDot(question.correct_answer)}</td>
                    <td style="width: 60%;">${numberWithDot(question.wrong_answer)}</td>
                    </tr>`;
    });
    ...
    let questionsHtml = `
        ...
        <table style="width: 100%; border-collapse: collapse;">
            <tbody>
            <tr>
            <th style="width: 60%; text-align:left">Soru</th>
            <th style="width: 13.3%; text-align:left">Level</th>
            <th style="width: 13.3%; text-align:left">Doğru Cevap</th>
            <th style="width: 13.3%; text-align:left">Yanlış Cevap</th>
            </tr>
            ${questionsBody}
            </tbody>
        </table>`;
